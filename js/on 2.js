...
function setupAmbilight() {
	...
	if(canvas.width==0) {
		...
		videoAmbiPlayer.removeEventListener("play", addCallback(), false);
		videoAmbiPlayer.addEventListener("play", addCallback(), false);
...
function timerCallback() {
	if (videoAmbiPlayer.paused || videoAmbiPlayer.ended || !ambilightState) {
		console.log("Ambilight: callBack off, ambilightState: " + ambilightState + videoAmbiPlayer.paused + videoAmbiPlayer.ended);
		callBackOn = 0;
		return;
	}
